...
%               Input_PCE{1}=[-1, 0.5]
%dd=par(2);
%               Normal with mean 1 and Variance 0.25
load('PCE_b');
%               Input_PCE{2}=PCE_tau; given in the file PCE_tau.mat
% Btilde_1=par(3);
%               Uniform in [0.5,1.5]
%               Input_PCE{3}=[1, 0.5]
...
dd=par(2); %INPUT discrete delays row vector dd=[d_{1},...,d_{q}]>=0
Input_PCE{2}=[1, 0.5];

Btilde{1}=@(t,d,par) par(3); %INPUT: dxd matrix or call to Btilde1.m
